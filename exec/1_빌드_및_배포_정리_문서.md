...
- 해당 프로그램을 원할하게 실행하기 위해서는 파이썬 서버와 올라마, 유니티를 모두 실행해야 합니다.

- 패키지 파일 내부의 'RunNewEden.bat' 파일을 실행하면 파이썬 서버(가상환경 세팅 및 의존성  자동 설치)와 올라마, 유니티를 모두 실행하며 모델 다운로드도 자동으로 실행합니다.
